    ...
    dbl * __restrict__ const host_matrix, const uint n) noexcept
{
    ...
    const dim3 block = dim3(8U, 4U), thread = dim3(8U, 4U);
    ...
    for (uint i = 0; i < n - 1; ++i)
    {
        ...
        if (max_idx != i)
        {
            row_switching<<<32U, 32U>>>(device_matrix, n, i, max_idx);
        }
        column_to_null_down<<<block, thread>>>(device_matrix, n, i);
    }
    for (uint i = n - 1; i > 0; --i)
    {
        column_to_null_up<<<32U, 32U>>>(device_matrix, n, i);
    }

    solve<<<32U, 32U>>>(device_matrix, n);
