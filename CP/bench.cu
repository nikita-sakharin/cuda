...
__global__ static void initState(ullong);
...
__global__ static void updatePoint(float2 * __restrict__,
    float2 * __restrict__, float2 * __restrict__, float2 * __restrict__, uint);
...
__device__ static void updateVelocityPosition(float2 * __restrict__,
    ...
    const float2 * __restrict__, uint, uint);
...
__device__ static curandState_t state;
...
int main(int argc, char *argv[])
{
    ...
    initPoint<<<block1, thread1>>>(pos_global, lbest_global, N,
        { -0.0625F, -0.0625F / WIDTH * HEIGHT },
        { 0.0625F, 0.0625F / WIDTH * HEIGHT });
...
__global__ static void initState(const ullong seed)
{
    curand_init(seed, seed, seed, &state);
}

__global__ static void initPoint(float2 * __restrict__ const pos,
    float2 * __restrict__ const lbest, const uint n,
    ...
    float2 * __restrict__ const lbest, const uint n)
{
    ...
    for (uint i = idx; i < n; i += offset)
    {
        ...
        updateVelocityPosition(pos, v, f, lbest, n, i);
    ...
    const uint n, const ullong seed)
{
    ...
    curandErrorCheck(curandGenerateUniform(generator, (float *) pos,
...
__device__ static void updateVelocityPosition(float2 * __restrict__ const pos,
    ...
    const float2 * __restrict__ const lbest, const uint n, const uint i)
{
    const flt k_1 = curand_uniform(&state), k_2 = curand_uniform(&state);
